...
openAI gym의 CartPole-v0 환경에 DQN을 적용시켜보는
...
def policy(done, step_count = 0, next_state = 0):
    if step_count >= 199:
        return 100
    if done:
        return -100
    return (0.26 - abs(next_state[2])) / 0.13 * 100
...
class DQN:
    ...
    def _build_network(self, h_size=hidden_layer_size, l_rate=l_rate):
        ...
        with tf.variable_scope(self.net_name):
            ...
            W2 = tf.get_variable("W2", shape=[h_size, output_size],
                                 initializer=tf.contrib.layers.xavier_initializer())
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for episode in range(max_episodes):
            ...
            if len(log) >= 100 and np.mean(log[(len(log)-consecutive_trial):len(log)]) >= 195:
                break
